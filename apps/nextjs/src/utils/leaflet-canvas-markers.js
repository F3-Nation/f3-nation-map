...
function layerFactory(L) {
  var CanvasIconLayer = (L.Layer ? L.Layer : L.Class).extend({
    ...
    redraw: function (clear = true) {
      CANVAS_LOG && console.log("CanvasIconLayer redraw");
      this._redraw(clear);
    },

    //Multiple layers at a time for rBush performance
    addMarkers: function (markers) {
      CANVAS_LOG && console.log("CanvasIconLayer addMarkers");
      var self = this;
      var tmpMark = [];
      var tmpLatLng = [];
    ...
    addLayers: function (layers) {
      CANVAS_LOG && console.log("CanvasIconLayer addLayers");
    ...
    onAdd: function (map) {
      CANVAS_LOG && console.log("CanvasIconLayer onAdd");
      this._map = map;
      ...
      // Call _reset immediately to set up the canvas correctly
      // Otherwise the markers are in the wrong position to start
      this._reset();
    },
    ...
    addTo: function (map) {
      CANVAS_LOG && console.log("CanvasIconLayer addTo");
      map.addLayer(this);
      return this;
    },

    clearLayers: function () {
      CANVAS_LOG && console.log("CanvasIconLayer clearLayers");
    ...
    _addMarker: function (marker, latlng, isDisplaying) {
      CANVAS_LOG && console.log("CanvasIconLayer _addMarker");
      var self = this;
    ...
    _drawMarker: function (marker, pointPos) {
      // LOG && console.log("CanvasIconLayer _drawMarker");
      var self = this;
      ...
      pointPos.x += mapSize.x * EXTEND_FACTOR;
      pointPos.y += mapSize.y * EXTEND_FACTOR;
    ...
    _drawImage: function (marker, pointPos) {
      // LOG && console.log("CanvasIconLayer _drawImage");
    ...
    _reset: function () {
      CANVAS_LOG && console.log("CanvasIconLayer _reset");
      var mapSize = this._map.getSize();
      var topLeft = this._map
        .containerPointToLayerPoint([0, 0])
        .subtract([mapSize.x * EXTEND_FACTOR, mapSize.y * EXTEND_FACTOR]);

      L.DomUtil.setPosition(this._canvas, topLeft);

      var size = this._map.getSize();

      this._canvas.width = size.x * (1 + 2 * EXTEND_FACTOR);
      this._canvas.height = size.y * (1 + 2 * EXTEND_FACTOR);

      this._redraw();
    },

    _redraw: function (clear) {
      CANVAS_LOG && console.log("CanvasIconLayer _redraw, clear", clear);
      var self = this;
      ...
      var extendedBounds = mapBounds.pad(EXTEND_FACTOR);

      //Only re-draw what we are showing on the map.
      var mapBoxCoords = {
        minX: extendedBounds.getWest(),
        minY: extendedBounds.getSouth(),
        maxX: extendedBounds.getEast(),
        maxY: extendedBounds.getNorth(),
      };
    ...
    _initCanvas: function () {
      CANVAS_LOG && console.log("CanvasIconLayer _initCanvas");
    ...
    _executeListeners: function (event) {
      CANVAS_LOG && console.log("CanvasIconLayer _executeListeners");
    ...
    _onLayerDidMove: function () {
      CANVAS_LOG && console.log("CanvasIconLayer _onLayerDidMove");
      var self = this;
      var mapSize = self._map.getSize();

      self._canvas.width = mapSize.x * (1 + 2 * EXTEND_FACTOR);
      self._canvas.height = mapSize.y * (1 + 2 * EXTEND_FACTOR);

      self._canvas.style.transform =
        "translate(" +
        -mapSize.x * EXTEND_FACTOR +
        "px," +
        -mapSize.y * EXTEND_FACTOR +
        "px)";
