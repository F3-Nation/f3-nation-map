...
export const requestRouter = createTRPCRouter({
  ...
  submitDeleteRequest: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const [existingAO] = input.aoId
        ? await ctx.db
            .select()
            .from(schema.orgs)
            .where(
              and(
                eq(schema.orgs.id, input.aoId),
  ...
  submitUpdateRequest: publicProcedure
    ...
    .mutation(async ({ ctx, input: input }) => {
      ...
      // Immediately update if user has permission
      if (
        permissions.canEditRegion.success &&
        permissions.canEditEvent.success &&
        permissions.canEditLocation.success &&
        ctx.session?.user?.email
      ) {
