...
import type { UpdateRequestData, UpdateRequestResponse } from "./types";
import {
  createAO,
  getLocationIdsForAO,
  moveAOToRegion,
...
export const recordUpdateRequest = async (
  ctx: Context,
  updateRequest: Omit<Partial<UpdateRequestData>, "reviewedBy"> & {
    regionId: number;
    submittedBy: string;
...
export const handleMoveAOToDifferentRegion = async (
  ...
  const [ao] = await ctx.db
    ...
    .from(schema.orgs)
    .where(
      and(eq(schema.orgs.id, request.aoId), eq(schema.orgs.orgType, "ao")),
    );
  ...
  // 1. Move the AO to the new region
  await moveAOToRegion(ctx, {
    aoId: request.aoId,
    oldParentId: originalRegionId,
    newParentId: request.regionId,
  });
...
export const handleLegacyEdit = async (
  ...
  if (request.aoId == undefined) {
    ...
    if (request.regionId !== ao.parentId && ao.parentId !== null) {
      const { newLocationIds } = await moveAOToRegion(ctx, {
        aoId: request.aoId,
        oldParentId: ao.parentId,
        newParentId: request.regionId ?? 0,
      });
